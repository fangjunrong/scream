...
export default {
  ...
  methods: {
    ...mapActions('skeletonButtock', [
      'fetchSkeletonButtockFatigue'
    ]),
    async search() {
      const param = _.assign(this.filter, { pageSize: 10, pageNumber: 1 })
      const result = await this.fetchSkeletonButtockFatigue(param)
      if (result.code !== 200) {
    ...
    async getData(param) {
      return await this.fetchSkeletonButtockFatigue(param)
    },
    toDetail(item) {
      this.$router.push({
