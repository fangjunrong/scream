<template>
  <div class="buttockData">
    <div class="buttockData-title">
      <DetailTitle title="商业ET"/>
    </div>
    <div class="buttockData-filter">
      <!-- <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary buttockData-filter-search" value="查询" @click="search()"/>
      </el-form> -->
      <ul class="buttockData-filter-textShow">
        <li>工作效率（按月）： 同比增长 20% ， 环比增长 30%</li>
        <li>成本费用（按月）： 同比减少 30% ， 环比减少 40%</li>
      </ul>
    </div>
    <div class="buttockData-charts">
      ...
      <div class="buttockData-charts-container">
        <div class="chart1">
...
export default {
  ...
  methods: {
    ...mapActions('climb', [
      'fetchButtockDataList',
      'changeButtockData',
      'deleteButtockData'
    ]),
    async init() {
      // const result = await this.fetchbuttockDataList()
      // if (result.code !== 200) {
    ...
    async search() {
      ...
      const result = await this.fetchbuttockDataList(param)
      if (result.code !== 200) {
        this.$message.warning(result.message)
      }
      this.tableData = result.data.result
    ...
    async getData(param) {
      return await this.fetchbuttockDataList(param)
    },
    brokeClick(event) {
      this.$router.push({
        name: 'climbBootNum',
...
<style>
.buttockData .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
.buttockData .el-tabs__nav{
  float: none;
  text-align: center;
}
</style>
