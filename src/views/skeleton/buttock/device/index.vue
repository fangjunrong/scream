<template>
  <div class="buttockDevice">
    <div class="buttockDevice-title">
      <DetailTitle title="设备信息"/>
    </div>
    <div class="buttockDevice-filter">
      <el-form :inline="true">
        <el-form-item label="名称">
          <el-input v-model="filter.name" class="sinput"></el-input>
        </el-form-item>
        <input type="button" class="s-button-primary buttockDevice-filter-search" value="查询" @click="search()"/>
        <el-button type="info" class="buttockDevice-filter-add" @click="add()">添加</el-button>
      </el-form>
    </div>
    ...
    <div class="buttockDevice-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...mapActions('skeletonButtock', [
      ...
      'changeSkeletonButtockDevice',
      'deleteSkeletonButtockDevice',
      'fetchSkeletonButtockPersonList'
    ]),
    async init() {
      const result = await this.fetchSkeletonButtockDeviceList({
        pageNumber: 1,
        pageSize: 10
      })
      if (result.code !== 200) {
    ...
    async search() {
      ...
      const result = await this.fetchSkeletonButtockDeviceList(param)
      if (result.code !== 200) {
        this.$message.warning(result.message)
      }
      this.tableData = result.data.result
    ...
    async getData(param) {
      return await this.fetchSkeletonButtockDeviceList(param)
    },
    add() {
    ...
    async infoSubmit() {
      ...
      const result = await this.changeSkeletonButtockDevice(this.info.data)
      if (result.code !== 200) {
        this.$message.warning(result.message)
        return false
      }
    ...
    async deleteItem(item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const result = await this.deleteSkeletonButtockDevice({ id: item.id })
        if (result.code !== 200) {
          this.$message.warning(result.message)
          return false
        }
...
<style>
.buttockDevice .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
