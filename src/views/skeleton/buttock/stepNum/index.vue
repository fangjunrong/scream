<template>
  <div class="skeletonButtockStepNum">
    <div class="skeletonButtockStepNum-title">
      <DetailTitle :sub-title="filter.searchDate" title="步数"/>
    </div>
    <div class="skeletonButtockStepNum-filter">
      <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary skeletonButtockStepNum-filter-search" value="查询" @click="search()"/>
      </el-form>
    </div>
    <div class="skeletonButtockStepNum-table">
    ...
    <div class="skeletonButtockStepNum-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...mapActions('skeletonButtock', [
      'fetchSkeletonButtockStepsNum'
    ]),
    async search() {
      const param = _.assign(this.filter, { pageSize: 10, pageNumber: 1 })
      const result = await this.fetchSkeletonButtockStepsNum(param)
      if (result.code !== 200) {
    ...
    async getData(param) {
      return await this.fetchSkeletonButtockStepsNum(param)
    },
    toDetail(item) {
      this.$router.push({
...
<style>
.skeletonButtockStepNum .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
