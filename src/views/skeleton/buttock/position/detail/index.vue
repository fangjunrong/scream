...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      const result = await this.fetchSkeletonButtockPositionDetail({
        pageNumber: 1,
        pageSize: 10,
        sn: this.sn
      })
    ...
    async infoSubmit() {
      ...
      const result = await this.changeButtockPosition(this.info.data)
      if (result.code !== 200) {
        this.$message.warning(result.message)
        return false
      }
    ...
    async deleteItem(item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const result = await this.deleteButtockPosition({ id: item.id })
        if (result.code !== 200) {
          this.$message.warning(result.message)
          return false
        }
