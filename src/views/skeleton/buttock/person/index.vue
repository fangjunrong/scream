<template>
  <div class="buttockPerson">
    <div class="buttockPerson-title">
      <DetailTitle title="人员信息"/>
    </div>
    <div class="buttockPerson-filter">
      <el-form :inline="true">
        <el-form-item label="名称">
          <el-input v-model="filter.name" class="sinput"></el-input>
        </el-form-item>
        <input type="button" class="s-button-primary buttockPerson-filter-search" value="查询" @click="search()"/>
        <el-button type="info" class="buttockPerson-filter-add" @click="add()">添加</el-button>
      </el-form>
    </div>
    <div class="buttockPerson-table">
      ...
      <table v-for="item in tableData" :key="item.id" class="selftable selftable-body">
        <tr>
          ...
          <td width="10%">{{ item.education }}</td>
          <td width="20%">{{ item.deviceModel ? item.deviceModel.name + "：" + item.deviceModel.sn : '' }}</td>
    ...
    <div class="buttockPerson-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...mapActions('skeletonButtock', [
      'fetchSkeletonButtockPersonList',
      'changeSkeletonButtockPerson',
      'deleteSkeletonButtockPerson',
      'fetchSkeletonButtockPersonList'
    ]),
    async init() {
      const result = await this.fetchSkeletonButtockPersonList({
        pageNumber: 1,
        pageSize: 10
      })
      if (result.code !== 200) {
      ...
      const personList = await this.fetchSkeletonButtockPersonList({
        pageNumber: 1,
        pageSize: 100000
      })
    ...
    async search() {
      ...
      const result = await this.fetchSkeletonButtockPersonList(param)
      if (result.code !== 200) {
        this.$message.warning(result.message)
      }
      this.tableData = result.data.result
    ...
    async getData(param) {
      return await this.fetchSkeletonButtockPersonList(param)
    },
    add() {
    ...
    async infoSubmit() {
      ...
      const result = await this.changeSkeletonButtockPerson(this.info.data)
      if (result.code !== 200) {
        this.$message.warning(result.message)
        return false
      }
    ...
    async deleteItem(item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const result = await this.deleteSkeletonButtockPerson({ id: item.id })
        if (result.code !== 200) {
          this.$message.warning(result.message)
          return false
        }
...
<style>
.buttockPerson .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
