    ...
    <el-dialog :visible.sync="info.visible" :title="info.typeText">
      <el-form :model="info.data">
        ...
        <el-form-item :label-width="formLabelWidth" label="动力">
          <el-input v-model="info.data.power"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...mapActions('skeletonWaist', [
      ...
      'changeSkeletonWaistDevice',
      'deleteSkeletonWaistDevice',
      'fetchSkeletonWaistPersonList'
    ]),
    async init() {
      const result = await this.fetchSkeletonWaistDeviceList({
        pageNumber: 1,
        pageSize: 10
      })
      if (result.code !== 200) {
    ...
    async search() {
      ...
      const result = await this.fetchSkeletonWaistDeviceList(param)
      if (result.code !== 200) {
        this.$message.warning(result.message)
      }
      this.tableData = result.data.result
    ...
    async getData(param) {
      return await this.fetchSkeletonWaistDeviceList(param)
    },
    add() {
    ...
    async infoSubmit() {
      ...
      const result = await this.changeSkeletonWaistDevice(this.info.data)
      if (result.code !== 200) {
        this.$message.warning(result.message)
        return false
      }
    ...
    async deleteItem(item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const result = await this.deleteSkeletonWaistDevice({ id: item.id })
        if (result.code !== 200) {
          this.$message.warning(result.message)
          return false
        }
