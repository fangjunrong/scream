  ...
  <div class="waistPerson">
    ...
    <div class="waistPerson-filter">
      <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary waistPerson-filter-search" value="查询" @click="search()"/>
        <el-button type="info" class="waistPerson-filter-add" @click="add()">添加</el-button>
      </el-form>
    </div>
    ...
    <div class="waistPerson-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      const result = await this.fetchSkeletonWaistPersonList(param)
      if (result.code !== 200) {
        this.$message.warning(result.message)
      }
      this.tableData = result.data.result
    ...
    async getData(param) {
      return await this.fetchSkeletonWaistPersonList(param)
    },
    add() {
    ...
    async infoSubmit() {
      ...
      const result = await this.changeSkeletonWaistPerson(this.info.data)
      if (result.code !== 200) {
        this.$message.warning(result.message)
        return false
      }
    ...
    async deleteItem(item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const result = await this.deleteSkeletonWaistPerson({ id: item.id })
        if (result.code !== 200) {
          this.$message.warning(result.message)
          return false
        }
...
<style>
.waistPerson .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
