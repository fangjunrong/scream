<template>
  <div class="skeletonArmDevice">
    <div class="skeletonArmDevice-title">
      <DetailTitle :sub-title="'设备序列号:' + filter.deviceId" title="开机次数详情"/>
    </div>
    <div class="skeletonArmDevice-filter">
      <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary skeletonArmDevice-filter-search" value="查询" @click="search()"/>
      </el-form>
    </div>
    <div class="skeletonArmDevice-table">
    ...
    <div class="skeletonArmDevice-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...mapActions('skeletonArm', [
      'fetchSkeletonArmBootNumDetail'
    ]),
    async search() {
      const param = _.assign(this.filter, { pageSize: 10, pageNumber: 1 })
      const result = await this.fetchSkeletonArmBootNumDetail(param)
      if (result.code !== 200) {
    ...
    async getData(param) {
      return await this.fetchSkeletonArmBootNumDetail(param)
    },
...
<style>
.skeletonArmDevice .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
