  ...
  <div class="armPositionDetail">
    ...
    <div class="armPositionDetail-filter">
      <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary armPositionDetail-filter-search" value="查询" @click="search()"/>
      </el-form>
    </div>
    <div class="armPositionDetail-table">
    ...
    <div class="armPositionDetail-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...mapActions('skeletonArm', [
      'fetchSkeletonArmPositionDetail'
    ]),
    async init() {
      this.sn = this.$route.params.sn
      const result = await this.fetchSkeletonArmPositionDetail({
        pageNumber: 1,
        pageSize: 10,
        sn: this.sn
      })
    ...
    async search() {
      ...
      const result = await this.fetchSkeletonArmPositionDetail(param)
      if (result.code !== 200) {
        this.$message.warning(result.message)
      }
      this.tableData = result.data.result
    ...
    async infoSubmit() {
      ...
      const result = await this.changeArmPosition(this.info.data)
      if (result.code !== 200) {
        this.$message.warning(result.message)
        return false
      }
    ...
    async deleteItem(item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const result = await this.deleteArmPosition({ id: item.id })
        if (result.code !== 200) {
          this.$message.warning(result.message)
          return false
        }
...
<style>
.armPositionDetail .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
