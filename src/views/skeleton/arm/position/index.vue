  ...
  <div class="skeletonArmPosition">
    ...
    <div class="skeletonArmPosition-filter">
      <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary skeletonArmPosition-filter-search" value="查询" @click="search()"/>
      </el-form>
    </div>
    <div class="skeletonArmPosition-table">
    ...
    <div class="skeletonArmPosition-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      const result = await this.fetchSkeletonArmPositionList(param)
      if (result.code !== 200) {
    ...
    async infoSubmit() {
      ...
      const result = await this.changeSkeletonArmPosition(this.info.data)
      if (result.code !== 200) {
        this.$message.warning(result.message)
        return false
      }
    ...
    async deleteItem(item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const result = await this.deleteSkeletonArmPosition({ id: item.id })
        if (result.code !== 200) {
          this.$message.warning(result.message)
          return false
        }
...
<style>
.skeletonArmPosition .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
