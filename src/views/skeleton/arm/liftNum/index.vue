<template>
  <div class="skeletonArmLiftNum">
    <div class="skeletonArmLiftNum-title">
      <DetailTitle :sub-title="filter.searchDate" title="托举次数"/>
    </div>
    <div class="skeletonArmLiftNum-filter">
      <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary skeletonArmLiftNum-filter-search" value="查询" @click="search()"/>
      </el-form>
    </div>
    <div class="skeletonArmLiftNum-table">
      ...
      <table v-for="item in tableData" :key="item.id" class="selftable selftable-body">
        <tr>
          ...
          <td width="30%">{{ item.liftNum }}</td>
          <td width="20%">{{ item.createTime }}</td>
        </tr>
      </table>
    </div>
    <div class="skeletonArmLiftNum-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...mapActions('skeletonArm', [
      'fetchSkeletonArmLiftNum'
    ]),
    async search() {
      const param = _.assign(this.filter, { pageSize: 10, pageNumber: 1 })
      const result = await this.fetchSkeletonArmLiftNum(param)
      if (result.code !== 200) {
    ...
    async getData(param) {
      return await this.fetchSkeletonArmLiftNum(param)
    },
    toDetail(item) {
      this.$router.push({
...
<style>
.skeletonArmLiftNum .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
