  ...
  <div class="skeletonArmLiftDetail">
    ...
    <div class="skeletonArmLiftDetail-filter">
      <el-form :inline="true">
        ...
        <input type="button" class="s-button-primary skeletonArmLiftDetail-filter-search" value="查询" @click="search()"/>
      </el-form>
    </div>
    <div class="skeletonArmLiftDetail-table">
      <el-tabs tab-position="top" style="height: 200px;">
        <el-tab-pane label="图表">
          <div class="skeletonArmLiftDetail-charts-container">
            <div class="chart1">
    ...
    <div class="skeletonArmLiftDetail-pagination">
      <el-pagination
...
export default {
  ...
  methods: {
    ...mapActions('skeletonArm', [
      'fetchSkeletonArmLiftNumDetail'
    ]),
    async init() {
      const result = await this.fetchSkeletonArmLiftNumDetail({
        pageNumber: 1,
        pageSize: 10
      })
      if (result.code !== 200) {
    ...
    initCharts() {
      ...
      this.brokeline.series[0].data = _.map(this.tableData, 'liftNum')
    },
    async search() {
      const param = _.assign(this.filter, { pageSize: 10, pageNumber: 1 })
      const result = await this.fetchSkeletonArmLiftNumDetail(param)
      if (result.code !== 200) {
        this.$message.warning(result.message)
      }
      this.tableData = result.data.result
    ...
    async getData(param) {
      return await this.fetchSkeletonArmLiftNumDetail(param)
    },
...
<style>
.skeletonArmLiftDetail .el-form-item__label{
  font-weight: bold;
  font-size: 14px;
  color: #00F0FA;
}
